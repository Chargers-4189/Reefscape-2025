...
public class AlignStationPos extends Command {
  ...
  public AlignStationPos(SwerveSubsystem swerve, Vision vision) {
    this.swerve = swerve;
    this.vision = vision;
  ...
  public boolean isFinished() {
    if (vision.getBackTagPose() != null) {
      return (
        vision.getBackTagPose().getX() <= 0.15 &&
        vision.getBackTagPose().getY() <= 0.05
      );
    }
